...
#
# funkcia ktora prekresli rucicky
#
def aktualizuj():
    canvas.delete('rucicky')   # zmaze iba rucicky, nie cifernik
    ...
    canvas.create_line(s1, s2, s1+dlha_ruc*math.cos(uhol_minuta), s2+dlha_ruc*math.sin(uhol_minuta), width=hrubka_h, fill='black', tags='rucicky')

    #
    # sekundy
    #
    uhol_sekunda = math.radians(cas.second * 6 - 90)   #-90 lebo posuvame do hora na 12 hodinu z 3
    canvas.create_line(s1, s2, s1+dlha_ruc*math.cos(uhol_sekunda), s2+dlha_ruc*math.sin(uhol_sekunda), width=hrubka_s, fill='black', tags='rucicky')

    #
    # hodiny
    #
    uhol_hodina = math.radians(cas.hour * 30 + cas.minute*0.5 - 90)   #-90 lebo posuvame do hora na 12 hodinu z 3
    canvas.create_line(s1, s2, s1+kratka_ruc*math.cos(uhol_hodina), s2+kratka_ruc*math.sin(uhol_hodina), width=hrubka_h, fill='black', tags='rucicky')

    #
    # znova zavolaj po 1000ms (1 sekunda)
    #
    win.after(1000, aktualizuj)
